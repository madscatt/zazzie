...
def get_match_point(other_self):
    '''
    **Get Match Point** is the Match Point Analysis method that calculates the match point from SANS contrast variation data. I(0) is divided by the concentration (c) to normalize for differences in concentration at each contrast. To make the relationship between I(0)/c and fraction D2O linear, sqrt[I(0)/c] is calculated and the sign in changed for the D\ :sub:`2`\ O values that are less than the match point where I(0) ~ 0. Thus, an initial guess for the match point must be provided. This can usually be guessed from the I(0) vs fraction D\ :sub:`2`\ O data.
