...
class Test_Em_to_Sas_Filter(MockerTestCase):
   ...
   'path does not exist']
    ...
    def test_4(self):
        ...
        '''
        self.inputpath= os.path.join(module_data_path,'no_write_permission')
        return_error = gui_mimic_em_to_sas.run_module(self, file_check=True)

        ''' check for path error '''
        expected_error = ['permission error in input file path ' + self.inputpath + '  [code = TrueTrueFalse]', 'write permission not allowed']
        assert_equals(return_error, expected_error)      
