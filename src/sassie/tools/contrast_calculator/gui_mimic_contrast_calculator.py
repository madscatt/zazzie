...
import sassie.interface.contrast_calculator.contrast_calculator_filter as contrast_calculator_filter
...
#    print 'variables in run module: ', self.variables
    ...
    error1 = contrast_calculator_filter.check_numfiles(
        self.variables['numfiles'][0])
    error2 = contrast_calculator_filter.check_numsolvcomp(
        self.variables['numsolv'][0])
    error3 = contrast_calculator_filter.check_numchemcomp(
        self.variables['number_of_chemicals'][0])
    ...
    if (int(self.numsolv) > 0):
        error, self.solv_formula = input_filter.check_and_convert_formula(
            self.solv_comp)
        if (len(error) > 0):
            ...
            for i in range(int(self.numsolv)):
                ...
                self.solvvariables.append(
                    [self.solv_formula[i], self.solv_conc[i]])
            error = contrast_calculator_filter.check_solvvariables(
                self.solvvariables)
            if len(error) > 0:
                print('error = ', error)
                if not (self.testflag):
    ...
    if (int(self.number_of_chemicals) > 0):
        error, self.formulas = input_filter.check_and_convert_formula(
            self.formula_array)
        if (len(error) > 0):
            print('error = ', str(error))
            if not (self.testflag):
            ...
            for i in range(int(self.number_of_chemicals)):
                ...
                self.chemvariables.append([this_chemical_formula, this_number_exchangeable_hydrogens,
                                          this_fraction_exchangeable_hydrogens, this_mass_density])
            error = contrast_calculator_filter.check_chemvariables(
                self.chemvariables)
            if (len(error) > 0):
                print('error = ', str(error))
                if not (self.testflag):
    ...
    this_contrast_calculator.main(
        self.variables, self.ivariables, self.solvvariables, self.chemvariables, txtQueue)
