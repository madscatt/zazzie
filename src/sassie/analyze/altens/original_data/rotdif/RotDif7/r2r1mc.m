function [me_mc,sd_mc]=r2r1mc(PAR0,VARmin,invCov,mc_max,mcmode)
%-----------------------------------------------------------------------------
% df-dec-98	
...
%dchi2=[2.71 4.61 6.25 7.78 9.24 10.6];  	% 90.0% conf.level
 ...
 if mcmode==1,                                    %visualization = off
   ...
   while mc_i<=mc_max,                          %MC-simulation
     ...
     for itmp=1:mc_max,
        dd=d0.*sim(itmp,:);
        dPAR=PAR0.*dd;
        if PAR0(2)+dPAR(2)>=0 & dPAR*invCov*dPAR' <= chi2bound;
          border=ones(1,npar);
          for iii=1:npar,
 ...
 figure (12)
    ...
    if kNaN==0,   	  %exclude NaN from the show
       axis([-100*d0(2) 100*d0(2) -100*d0(1) 100*d0(1)]);
       plot(100*RES_mc(:,2)/PAR0(2),100*RES_mc(:,1)/PAR0(1),'.');
       hold on
       plot([-100*d0(2) 100*d0(2)],[0 0],'w--');
