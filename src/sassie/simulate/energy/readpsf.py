...
#       1 FALA 1    ALA  CAY  CT3   -0.270000       12.0110           0
	...
	st=string.split(infile[offset1]) ; num_atoms=locale.atoi(st[0]) ; print 'natoms = ',num_atoms
	offset2=offset1+num_atoms+2
	for i in xrange(offset1+1,offset1+1+num_atoms):
		tal=string.split(infile[i])
		segments.append(tal[1]) ; atoms.append([tal[4],tal[5]]) ; charge.append(tal[6]) ; mass.append(tal[7])

	st=string.split(infile[offset2]) ; num_bonds=locale.atoi(st[0]) ; print 'nbonds = ',num_bonds
...
#	print 'numbondlines = ',numbondlines+leftover 
	for i in xrange(offset2+1,offset2+1+numbondlines+leftover):
		tbl=string.split(infile[i]) 
		for j in xrange(0,len(tbl),2):
			bonds.append([tbl[j],tbl[j+1]])

#	print bonds

	offset3=offset2+numbondlines+2+leftover
	st=string.split(infile[offset3]) ; num_angles=locale.atoi(st[0]) ; print 'nangles = ',num_angles
	...
	for i in xrange(offset3+1,offset3+1+numanglelines+leftover):
		tal=string.split(infile[i])
		for j in xrange(0,len(tal),3):
			angles.append([tal[j],tal[j+1],tal[j+2]])

#	print angles
	
	offset4=offset3+numanglelines+2+leftover	
	st=string.split(infile[offset4]) ; num_dihedrals=locale.atoi(st[0]) ; print 'dihedrals = ',num_dihedrals
	...
	for i in xrange(offset4+1,offset4+1+numdihedrallines+leftover):
		tdl=string.split(infile[i])
		for j in xrange(0,len(tdl),4):
			dihedrals.append([tdl[j],tdl[j+1],tdl[j+2],tdl[j+3]])
#	print dihedrals


	st=string.split(infile[offset5]) ; 
	if(len(st)>0):
	...
	for i in xrange(offset5+1,offset5+1+numimproperlines+leftover):
		til=string.split(infile[i])
		for j in xrange(0,len(til),4):
