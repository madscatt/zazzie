...
# now get the 1-3 list (difference of 1-2 and 1-2 plus 1-3)
	...
	for i in range(len(exclusionlist)):
		lex=[]
		for j in range(len(exclusionlist[i][1])):
...
### then put the 1-2 for each atom in the 1-3 list (this is the overcomplete 1-4) 
	...
	overonefour=[]
	for i in range(len(onethree)):
		lex=[]
		for j in range(len(onethree[i][1])):
			tatom=onethree[i][1][j]
			# now find the index and the bonded atoms for tatom in the 1-2 list
			for k in range(len(bondlist)):
				batom=bondlist[k][0]
				if(tatom==batom):
					for m in range(len(bondlist[k][1])):
...
### then remove the atoms in the original 1-2 list for that atom to get the 1-4 list
	...
	onefourlist=[] ; n14=0
	for i in range(len(overonefour)):
		lex=[]
		tatom=overonefour[i][0]
		for k in range(len(overonefour[i][1])):
