...
'''
import os,sys,string,locale,numpy
...
# check and see if the number of values found matches the number of basis definitions that are needed

	if(len(basis) != num_basis):
		error.append('incorrect number of basis values entered : '+basis_string)
	...
	try:
		...
		error.append('unable to read file : '+pdbfile+' while checking basis')
		return error,mask_array
...
# check and see if a keyword was supplied, if this passes then you get the atom array for that segment
	...
	if(len(segment_basis) > 0 and len(segment_basis) != num_basis):
		error.append('can not use a mixture of keyword basis definitons and atom names')
		return error,mask_array
...
# check and see if the atoms requested for each basis exist over the ranges requested

	if(len(residue) != num_basis):
		error.append('number of alignment ranges does not match the number of basis ranges')
		return error,mask_array
...
#
	...
	unique_segment_list=[] ; natoms_per_segment=[] ; natoms_last_segment = 0
	...
	if(num_basis != len(unique_segment_list)):
		error.append('number of basis ranges does not equal the number of unique segment names in pdb file')
		return error,mask_array
	...
	for i in xrange(num_basis):
		...
		for j in xrange(natoms_per_segment[i]):
			...
			if (j==0): 
				lastres=thisres-1
				if thisres != 1:
					error.append('the first residue needs to be 1 for each segment in PDB file')
					return error,mask_array
		...
		if(numpy.sum(preliminary_mask_array) < 3):
			error.append('you need to specify at least 3 atoms to form a basis for segment = '+unique_segment_list[i])
