...
def check_numfiles(numfiles):
    ...
    if (intvalue < 0):
        error.append(
            "The number of input files must be greater than or equal to zero.")
        return error
    return error
...
def check_ivariables(path, ivariables):
    ...
    for i in range(len(allfilenames)):
        ...
        if allIsFasta[i] != '1':
            if (ev == 1 and value == 0):
                error.append('Input file "'+pdbfile +
                             '" is not in valid PDB format. FASTA?')
                return error
...
#            print 'len(matching: ',len(matching)
            ...
            if len(matching) == 0:
                error.append(
                    "Input file "+str(i+1)+" does not contain a valid dna, rna or protein molecule.")
                return error
#            elif len(matching) > 1:
...
#           DNA can be classified as 'rna' here and get through because only THY is classified as 'dna' in SasMol
            if ithtype not in matching:
                error.append(
                    'Input file '+str(i+1)+' contains residues that do not match the moltype "'+str(ithtype)+'"')
                return error

        else:
            fastaseq = contrast_helper.FASTA_sequence(pdbfile)
            if not fastaseq:
                error.append('The FASTA file "'+pdbfile +
                             '" appears to be empty.')
                return error
            ...
            if allmoltype[i] == 'dna':
                ...
                error.append('The molecule type for input file ' +
                             str(i+1)+' must be DNA, RNA or protein.')
                return error
...
#           User beware: DNA can be called 'protein' here and get through.
            for k in fastaseq:
                if k not in fastadict:
                    error.append(
                        "Input file "+str(i+1)+" contains the non-"+allmoltype[i]+" residue "+k+".")
                    return error
    ...
    for i in range(len(allnumunits)):
        try:
            ...
            error.append(
                'The number of units of each molecule for input file '+str(i+1)+' must be an integer.')
            return error
        else:
            if intvalue < 1:
                error.append(
                    'The number of units of each molecule for input file '+str(i+1)+' must be at least 1.')
                return error
    ...
    for i in range(len(allfracdeut)):
        try:
            ...
            error.append('The deuteration fraction of input file ' +
                         str(i+1)+' must be a number.')
            return error

        if floatvalue < 0 or floatvalue > 1:
            error.append('The deuteration fraction of input file ' +
                         str(i+1)+' must be between 0.0 and 1.0')
            return error
...
#    chemcomps=[]
    ...
    for i in range(len(hexchem)):
        try:
            ...
            error.append(
                'The number of exchangeable hydrogens for additional component '+str(i+1)+' must be an integer.')
            return error
        else:
            if intvalue < 0:
                error.append('The number of exchangeable hydrogens for additional component ' +
                             str(i+1)+' must be greater than or equal to zero.')
                return error
    ...
    for i in range(len(fexchchem)):
        try:
            ...
            error.append(
                'The fraction of exchangeable hydrogens for additional component '+str(i+1)+' must be a number.')
            return error
        else:
            if (floatvalue < 0.0 or floatvalue > 1.0):
                error.append('The fraction of exchangeable hydrogens for additional component ' +
                             str(i+1)+' must be between 0.0 and 1.0.')
                return error
    ...
    for i in range(len(chemdens)):
        try:
            ...
            error.append('The mass density of additional component ' +
                         str(i+1)+' must be a number.')
            return error
        else:
            if (floatvalue <= 0.0):
                error.append('The mass density of additional component ' +
                             str(i+1)+' must be greater than 0.')
                return error
...
#    solvcomps=[]
    ...
    for i in range(len(solvconcs)):
        try:
            ...
            error.append('The concentration of solvent component ' +
                         str(i+1)+' must be a number.')
            return error
        else:
            if floatvalue <= 0.0:
...
#            return error
    ...
    if (numfiles + numformulas < 1):
        error.append(
            "You must enter either one file (PDB or Fasta) or one chemical formula")
        return error
...
#        return error
    ...
    if (100 % d2ostep != 0):
        error.append(
            'D2O step size must divide 100% evenly (1%, 2%, 5%, etc.)')
        return error
...
#        return error
    ...
    if (fexchp < 0.0 or fexchp > 1.0):
        error.append(
            'The fraction of exchangeable protein hydrogens must be between 0.0 and 1.0.')
        return error
...
#        return error
    ...
    if (fexchn < 0.0 or fexchn > 1.0):
        error.append(
            'The fraction of exchangeable nucleic acid hydrogens must be between 0.0 and 1.0.')
        return error
