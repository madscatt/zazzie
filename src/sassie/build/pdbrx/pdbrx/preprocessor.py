...
def get_user_segmentation(mvars, mol):
    """
    ...
    resid_descriptions = segname_utils.create_residue_descriptions(mol)

    if mvars.ui_type == 'terminal':
        ...
        #        mol.segnames(), resid_descriptions, json, logger).get_segment_starts()
...
def get_user_fasta_sequence(mvars, segname, moltype):
    """
...
def get_biological_unit_transform(mvars, mol):
    """
...
def handle_sassie_web_edit_options(mvars, mol, pdbscan_report):
    ...
    self.logger.info('UI_TYPE = ' + mvars.gui)
    ...
    while not accepted_segmentation:
        ...
        ### RIPPED OUT ALL THE REST OF THE ORIGINAL LOGIC
        ### RIPPED OUT ALL THE REST OF THE ORIGINAL LOGIC
        ### RIPPED OUT ALL THE REST OF THE ORIGINAL LOGIC
...
# HERE
    ...
    def user_edit_options(mvars, mol, pdbscan_report):
        """
        ...
        if mvars.gui == 'terminal':
            ...
            handle_terminal_edit_options(mvars, mol)

        elif mvars.gui == 'sassie_web':

            handle_sassie_web_edit_options(mvars, mol, pdbscan_report)
            logger.info('I SHOULD QUIT HERE')
            logger.info('I SHOULD QUIT HERE')
            logger.info('I SHOULD QUIT HERE')
