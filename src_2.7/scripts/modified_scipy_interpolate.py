...
#from sassie.sasmol.sasmath import matrix_multiply
import sasmol.sasmath.matrix_multiply as matrix_multiply
...
def _dot0(a, b):
    ...
    if b.ndim <= 2:
        return matrix_multiply(a, b)[1]
    else:
        axes = range(b.ndim)
        axes.insert(-1, 0)
        axes.pop(0)
        return matrix_multiply(a, b.transpose(axes))[1]
...
def _find_smoothest(xk, yk, order, conds=None, B=None):
    ...
    V2 = vh[-ind:,:].T
    V1 = vh[:-ind,:].T
    A = matrix_multiply(J.T,J)[1]
    tmp = matrix_multiply(V2.T,A)[1]
    Q = matrix_multiply(tmp,V2)[1]
    p = np.dual.solve(Q,tmp)
    tmp = matrix_multiply(V2,p)[1]
    tmp = np.eye(N+K) - tmp
    tmp = matrix_multiply(tmp,V1)[1]
    tmp = matrix_multiply(tmp,np.diag(1.0/s))[1]
    tmp = matrix_multiply(tmp,u.T)[1]
    return _dot0(tmp, yk)
