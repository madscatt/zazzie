...
class Chain_Info_Reconciler():
    ...
    def get_common_chains(self):
        ...
        head_seq_chains = set(head_info.sequence.keys() +
                              head_info.heterogens.keys())
    ...
    def compare_sequences(self, model_no=1):
        ...
        for chain in self.common_chains:
            ...
            agree, removed_resids = self.compare_sequence(
                chain, model_no=model_no)
            ...
            if agree and removed_resids:
                ...
                logger.warning(
                    'Failure to match header and coordinate sequences in chain {0:s}'.format(chain))
    ...
    def check_missing(self, model_no=1):
        ...
        if head_chains != coord_chains:
            ...
            non_common_chains = set_head_chains.symmetric_difference(
                set_coord_chains)
    ...
    def update_coord_missing(self, model_no=1):
        ...
        for chain, residues_same in agree.items():
            ...
            if residues_same:
                ...
                only_missing_head = head_miss_resids.difference(
                    coord_miss_resids)
                only_missing_coord = coord_miss_resids.difference(
                    head_miss_resids)
                ...
                if only_missing_head:
                    ...
                    missing_after = [x for x in sorted(
                        only_missing_head) if x > last_coord_resid]
                    missing_before = [x for x in sorted(
                        only_missing_head, reverse=True) if x < first_coord_resid]
                    missing_mid = [x for x in sorted(
                        only_missing_head) if first_coord_resid < x < last_coord_resid]
                    ...
                    for resid in missing_after:
                        ...
                        coor_info.add_missing_resid(
                            chain, resid, resname, model_no=model_no)
                    ...
                    for resid in missing_mid:
                        ...
                        resname = head_missing[chain][resid]
                        txt = 'Header claims resid {0:d} is missing in chain {1:s} but residue {2:s} found in coordinates'.format(
                            resid, chain, resname)
            ...
            if chain in coor_hets:
                ...
                for resid, resname in hets.iteritems():
                    ...
                    if (resid in coor_hets[chain]) and (resname != coor_hets[chain][resid]):
                        ...
                        logger.warning(txt.format(
                            resname, coor_resname, chain, resid))
    ...
    def compare_sequence(self, chain, model_no=1):
        ...
        if chain in head_info.sequence:
            ...
            edited_seq, removed_resids = self.remove_unknown_from_sequence(
                coor_seq)
