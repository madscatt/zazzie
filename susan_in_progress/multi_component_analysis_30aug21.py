...
import read_contrast_output_files_30aug21 as read_contrast_output_files #this is older version of the method that isn't in the 2.0 format
...
class multi_component_analysis():
    ...
    def main(self, input_variables, multi_component_analysis_variables, txtOutput):
        ...
        self.process_input_variables(multi_component_analysis_variables)
    ...
    def process_input_variables(self,multi_component_analysis_variables):
        ...
        log.debug('multi_component_analysis_variables: %s' %(str(multi_component_analysis_variables)))
        log.debug('len(multi_component_analysis_variables): %i' % len(multi_component_analysis_variables))
...
#this will eventually be an if, elif depending on the chosen method
        if mvars.stoichiometry_flag == True:
            mvars.fraction_d2o = multi_component_analysis_variables[0]
            mvars.partial_specific_volume = multi_component_analysis_variables[1]
            mvars.izero = multi_component_analysis_variables[2]
            mvars.concentration = multi_component_analysis_variables[3]
...
#               mvars.delta_rho = delta_rho_read_from_file
                ...
                mvars.delta_rho = multi_component_analysis_variables[4]
...
#            avars.outfile.write('correlation coefficient: '+str(correlation_coefficient)+'\n')
        ...
        avars.outfile.write('Total Mw (kDa), weight_fraction_1, weight_fraction_2: '+str(total_molecular_weight)+'\t'+str(weight_fraction_1)+'\t'+str(weight_fraction_2))
        avars.outfile.close()
