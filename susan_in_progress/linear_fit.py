...
#                                                                      *      **
'''
    LINE FIT performs a weighted linear fit to a straight line y = a+ bx
    using the method of determinants. Adapted from the original FORTRAN
    program from Data Analysis and Error Reduction for the Physical Sciences,
    by Philip R. Bevington and D. Keith Robinson, McGraw-Hill, New York, NY
    2003, 1992, 1969 (Chapter 6).
    ...
    OUTPUTS:
        slope and error on slope
        intercept and error on intercept
        ycalc, calculated y values
        diff, difference between y and ycalc
        R value
        reduced chi-squared, number of data points -2 degrees of freedom
        

    Although a fit with weighting = 1.0/yerr**2 is assumed, mode is an
    input variable. Thus, this method applicable to fits with different
    weightings if desired.
    
    The value of mode determines the method of weighting for the fit:
    +1  instrumental weighting = 1.0/yerr**2
     0  no weighting = 1.0
    -1  statistical weighting (yerr = sqrt(y)) = 1.0/y
...
#calculate slope, intercept, errors, R value, reduced chi-squared
    if mode == -1:
        ...
        variance = (sumy2 + intercept*intercept*sumwt + slope*slope*sumx2 - 2.00*(intercept*sumy + slope*sumxy - intercept*slope*sumx))/(number_of_data_points - 2)
    elif mode == 1:
        variance = 1.0
    intercept_error = numpy.sqrt(variance*sumx2/determinant)
    slope_error = numpy.sqrt(variance*sumwt/determinant)
    r_value = (sumwt*sumxy - sumx*sumy)/numpy.sqrt(determinant*(sumwt*sumy2 - sumy*sumy)) 
...
#    print('x,y,yerr: ', x, y, yerr)
    ...
    slope, slope_error, intercept, intercept_error, r_value, reduced_chi_squared, ycalc, diff = line_fit(x,y,yerr,number_of_data_points, mode)
    ...
    outfile.write('slope: ' + str(round(slope,4)) + ' +/- ' + str(round(slope_error,4)) + '\n')
    outfile.write('intercept: ' + str(round(intercept,4)) + '+/-' + str(round(intercept_error,4)) + '\n')
...
#If we use absolute_sigma = True, the standard deviations are the same as the errors on the slope and intercept obtained above.
    popt, covariance = scipy.optimize.curve_fit(func, x, y, sigma=yerr, absolute_sigma=True)
...
#   compute one standard deviation of the molecular weights
        ...
        print('standard_deviation_slope, standard_deviation_intercept: ', standard_deviation_slope, standard_deviation_intercept)
        correlation_coefficient = covariance[0][1]/(standard_deviation_slope*standard_deviation_intercept)
