...
#import sassie.contrast.multicomponent_analysis.stoichiometry as stoichiometry
import stoichiometry as stoichiometry
#only stoichiometry is imported for now; others will be added later
...
def user_variables(self, **kwargs):
    ...
    self.output_file_name = os.path.join(path,'99_12_41.out')
#    self.input_file_name = os.path.join(path,'input_contrast.txt')
    self.input_file_name = ''

#some inputs will be common to more than one method; only use them once below for the method being run; comment out the rest
...
#    need Rg values
    ...
    self.delta_rho = [[u'-3.2', u'-5.7'], [u'1.6', u'0.26'], [u'0.031', u'-1.74']]  # 2 values for each contrast since there are 2 components
...
def run_module(self, **kwargs):
    ...
    svariables['path'] = (self.run_name,'string')
    ...
    try:
        if kwargs['file_check']:
            error = multi_component_analysis_filter.check_multi_component_analysis(self.variables, self.fraction_d2o, self.partial_specific_volume, self.izero, self.concentration, self.delta_rho)
    except:
            error = multi_component_analysis_filter.check_multi_component_analysis(self.variables, self.fraction_d2o, self.partial_specific_volume, self.izero, self.concentration, self.delta_rho, no_file_check="true")
