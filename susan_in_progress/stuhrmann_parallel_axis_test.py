...
# it returns fit, a vector with the cooeficients from the polynomial fit; correlation, the correlation matrix, and chi_squared 
        ...
        if n > order+1:
            for i in range(0,n):
                if sigY[i] > 0.0:
                    ...
                    chi_squared += (delta - Y[i])*(delta - Y[i])/sigY[i]/sigY[i]
    ...
    reduced_chi_squared,fit,correlation = polynomialFit(2,delta_rho_inverse,rg_squared,rg_squared_error,number_of_contrast_points)
