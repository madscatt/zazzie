...
from read_contrast_output_files_29jul21 import read_contrast_file
...
#0 is no and 1 is yes to reading the contrasts from a contrast calculator output file 
    ...
    path = ('./')
    fname = os.path.join(path,'99_12_41.out')    #output file (will always be specified) We may only want to ask for a file prefix like in contrast calculator?
...
#Specify whether contrast values will be read from a file.
    if read_from_file == 1:  
        ...
        fname1 = '' #won't be used but needs to be defined
        drho = [[u'-3.2',u'-5.7'],[u'1.6',u'0.26'],[u'0.031',u'-1.74']]  #2 values for each contrast point (since there are  2 components)
