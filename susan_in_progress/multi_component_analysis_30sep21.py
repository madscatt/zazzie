...
#unpack the variables that are unique to each method. This will become if, elif, etc. If some variables turn out to be needed for all methods, then they can be unpacked above before checking the flags. The read_from_file if statement may not be needed if the GUI is going to be populated with these values prior to running the main program.
...
#OR this will become its own initialization file just for stoichiometry that will be called 
        if mvars.stoichiometry_flag == True:
            if (mvars.run_name[-1] == '/'):
...
#TODO: write out the individual input variables on separate lines
